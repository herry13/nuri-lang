...
and resolve_follow ?visited:(accumulator=SetRef.empty) reference
    ...
    if SetRef.exists (fun r -> r = reference) accumulator then
        ...
        | ns, Val Link nextRef | ns, Val Basic Reference nextRef ->
            ...
            | Valid ref ->
                resolve_follow ~visited:(SetRef.add ref accumulator) nextRef
                               destReference !-ref store
            end
        | ns, value ->
            begin match ns @<< reference with
            | Invalid ->
...
let rec evalr ?follow_f:(follow=false) ?acc:(visited=SetRef.empty) s ns r =
    if SetRef.exists (fun rs -> r = rs) visited then
        ...
        | _, Undefined  ->
            error 526 ("'" ^ !^r ^ "' is not found.")

(* TODO: update semantics algebra in the documentation by adding this
         function *)
(** Evaluate a function *)
and evalf ?follow_r:(follow=false) ?acc:(visited=SetRef.empty) s ns f =
    let v = eval_function s ns f in
    match v with
    | Link r | Basic Reference r ->
...
let rec unary ?store:(s=[]) ?namespace:(ns=[]) map = function
    | Basic Reference r ->
        unary ~store:s ~namespace:ns map (evalr s ns r)
...
let rec binary ?store:(s=[]) ?namespace:(ns=[]) map v1 v2 = match v1, v2 with
    ...
    | Lazy f1, _ ->
        Lazy (fun ss nss ->
            binary ~store:ss ~namespace:nss map
                   (evalf ~follow_r:true ss nss f1) v2
        )

    | _, Lazy f2 ->
        Lazy (fun ss nss ->
            binary ~store:ss ~namespace:nss map v1
                   (evalf ~follow_r:true ss nss f2)
        )
...
let logic ?operator:(op="") ?store:(s=[]) ?namespace:(ns=[]) f_logic =
    binary ~store:s ~namespace:ns (fun xx yy -> match xx, yy with
        | Basic Boolean b1, Basic Boolean b2 ->
            Basic (Boolean (f_logic b1 b2))
...
let math ?store:(s=[]) ?namespace:(ns=[]) f_int f_float =
    binary ~store:s ~namespace:ns (fun xx yy -> match xx, yy with
        | Basic Int x  , Basic Float y ->
            Basic (Float (f_float (float_of_int x) y))

        | Basic Float x, Basic Int y   ->
            Basic (Float (f_float x (float_of_int y)))

        | Basic Float x, Basic Float y ->
            Basic (Float (f_float x y))

        | Basic Int x  , Basic Int y   ->
            Basic (Int (f_int x y))

        | _ ->
            error 527 ("Left or right operand is neither an integer nor a " ^
                       "float.")
    )
...
let add ?store:(s=[]) ?namespace:(ns=[]) =
    binary ~store:s ~namespace:ns (fun x y -> match x, y with
        ...
        | Basic Float f1, Basic Float f2 ->
            Basic (Float (f1 +. f2))

        | Basic Int i1, Basic Int i2 ->
            Basic (Int (i1 + i2))
        ...
        | _ ->
            error 528 "Operands of '+' are neither int, float, or string"
    )
;;
